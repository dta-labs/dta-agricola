/****************************************************************
 *                                                              *
 *               Sistemas DTA Serie AGxx v0.2.1 A               *
 *                                                              *
...
// Settings
const long config[] = {2, 3, 7, 52, 625, 1020596};          // Rx, Tx, Plots, Pais, Lada, Número
...
void setup() {
  ...
  apagarTodo();
  Serial.println(F(">>> DTA-Agrícola: Serie AGxx v0.2.1 A"));
...
void readEEPROM() {
  ...
  activationTime = eeVar.timeRiego;
  Serial.print(F("EEPROM ")); Serial.print(EEPROM.length());
  Serial.print(F(": ")); Serial.print(statusVar);
  Serial.print(F(" ")); Serial.print(plot);
  Serial.print(F(" ")); Serial.println((String)activationTime);
}

void updateEEPROM() {
  String stVar = (String(eeVar.status) == "ON") ? "ON" : "OFF";
  int pt = eeVar.plot >= plots ? plots : eeVar.plot;
  unsigned long tr = eeVar.timeRiego;
  if (statusVar != stVar || pt != plot || tr != activationTime) {
    statusVar.toCharArray(eeVar.status, 3);
    eeVar.plot = plot;
    eeVar.timeRiego = activationTime;
    EEPROM.put(0, eeVar);
    Serial.print(F("EEPROM ")); Serial.print(EEPROM.length());
    Serial.print(F(": ")); Serial.print(statusVar);
    Serial.print(F(" ")); Serial.print(plot);
    Serial.print(F(" ")); Serial.print((String)activationTime);
...
String httpRequest() {
  ...
  gprs.print("&tm=" + (String)activationTime);
