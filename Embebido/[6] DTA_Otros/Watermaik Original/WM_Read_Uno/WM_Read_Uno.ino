...
float readWMsensor() {  //read ADC and get resistance of sensor
  ...
  for (i = 0; i < num_of_read; i++) //the num_of_read initialized above, controls the number of read successive read loops that is averaged.
  {
    ...
    digitalWrite(A1, HIGH);   //Set pin 5 as Vs
    delayMicroseconds(90); //wait 90 micro seconds and take sensor read
    ARead_A1 += analogRead(A0); // read the analog pin and add it to the running total for this direction
    digitalWrite(A1, LOW);      //set the excitation voltage to OFF/LOW
    ...
    delayMicroseconds(90); //wait 90 micro seconds and take sensor read
    ARead_A2 += analogRead(A0); // read the analog pin and add it to the running total for this direction
  ...
  Serial.print(F(" L1: ")); Serial.print(ARead_A1); Serial.print(F(" L2: ")); Serial.print(ARead_A2); 
  Serial.print(F(" R1: ")); Serial.print(WM_ResistanceA); Serial.print(F(" R2: ")); Serial.print(WM_ResistanceB);
  Serial.print(F(" R: ")); Serial.print(WM_Resistance); Serial.print(F("ohms  ->  "));
